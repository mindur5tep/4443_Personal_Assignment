...
public class RecipeSteps extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonNext.setOnClickListener(v -> {
            // If on the last step and "Finish" is displayed, navigate back to RecipeActivity
    ...
    private void initPorcupine() {
        try {
            porcupineManager = new PorcupineManager.Builder()
                    .setAccessKey("YOUR_ACCESS_KEY_HERE")  // Replace with your actual access key
    ...
    private void updateStep() {
        if (steps != null && currentStep >= 0 && currentStep < steps.size()) {
            ...
            builder.setSpan(new RelativeSizeSpan(1.3f),
                    ...
                    Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            builder.append(":\n\n");
            // Remove any duplicate step label if present in the instruction
            String stepText = steps.get(currentStep).replaceFirst("^Step \\d+:\\s*", "");
            builder.append(stepText);
            textTv.setText(builder);

            // If we're at the last step, change the Next button text to "Finish"
            if (currentStep == steps.size() - 1) {
